...
def read_latest_tweet_date(company):
    ...
    if company == "Google":
        ...
        nvidia_latest = [
            dic["id"]
            for dic in json_data[company]
            if dic["day"] == str(date_today) or dic["day"] == str(date_yesterday)
        ]
...
def write_latest_tweet_date(company, data):
    ...
    for posting in data:
        existing_data[company].append({"day": str(date.today()), "id": posting[-2]})
...
def tweet_text(job_title, url, city, country, company, hashtag=""):
    msg = (
        f"New position for:\n"
        f"{job_title}\n"
        f"\n"
